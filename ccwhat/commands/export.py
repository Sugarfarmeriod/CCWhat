...
def _validate_session_ids(session_ids: tuple[str, ...]) -> list[str]:
    ...
    invalid_ids = [sid for sid in unique_ids if not SESSION_ID_RE.fullmatch(sid)]
    if invalid_ids:
...
@click.option(
    ...
    help="Output archive path. Defaults to ~/Downloads/ccwhat-exports/export-<timestamp>-<id>.tar.gz.",
)
@click.option(
    "--list",
    "list_only",
...
def export(
    ...
    if output is None:
        default_dir = Path.home() / "Downloads" / "ccwhat-exports"
    ...
    click.echo("\nTo share this package, send the archive. The recipient can import it with:")
    click.echo(f"  ccwhat import {output} --open")
