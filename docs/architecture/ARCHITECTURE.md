...
本文档面向参与 CCWhat 开发的协作者，说明项目的核心模块、数据流和边界。

## 项目定位

CCWhat 是一个 Coding Agent 观测诊断工具。当前它包含两条主要链路：
...
- `ccwhat/commands/web_server.py`
- `ccwhat/commands/setup.py`
- `ccwhat/commands/export_cmd.py`
- `ccwhat/commands/import_cmd.py`
...
- `ccwhat -- <target>` 中的 `<target>` 是 observed agent 的启动命令。
...
- 将原始网络记录保存到 `~/.ccwhat/raw-req-resp`。
...
```text
ccwhat web / ccwhat -- <agent>
  -> viewer/server.py
  -> ccwhat.adapters.registry
  -> AgentAdapter
  -> normalized session
  -> viewer/claude-log.html
```
...
```text
normalized session
  -> ccwhat.session_report.normalize
  -> ccwhat.session_report.core
  -> diagnosis_context
  -> ccwhat.analyzer.run_mc_analysis
  -> AnalyzerSpec
