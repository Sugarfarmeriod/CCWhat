...
V2.1.1 新增两个面向实际使用场景的交互功能：在 Viewer 中直接为 Codex / OpenCode session 命名，以及在抓包页面回放历史请求并编辑内容再次发送。
...
Session 重命名功能（含 Adapter title 元数据统一、`POST /api/session/<id>/rename` 接口、Codex SQLite 健壮性改进、时间戳归一化修复）由 [@tanzunsheng](https://github.com/tanzunsheng) 贡献，详见 [PR #2](https://github.com/PacemakerG/CCWhat/pull/2)。
