...
Claude Log Viewer SHALL 使用 Task Trace Overlay 表示当前 session 的可编辑任务划分，并保持 Raw Trace、Conversation 和 Turn 数据不可变。
...
Claude Log Viewer SHALL 允许用户通过按钮操作调整 Task 的起始 Turn 和结束 Turn。

#### Scenario: 设为 Task 起点
- **WHEN** 用户在编辑模式下选中某个 Task 内或相邻范围内的 Turn 并点击“设为 Task 起点”
- **THEN** 当前 Task 的 startTurnKey SHALL 更新为该 Turn
- **AND** Trace 树 SHALL 按新的 Task 范围刷新
- **AND** overlay SHALL 标记为未保存

#### Scenario: 设为 Task 终点
- **WHEN** 用户在编辑模式下选中某个 Task 内或相邻范围内的 Turn 并点击“设为 Task 终点”
- **THEN** 当前 Task 的 endTurnKey SHALL 更新为该 Turn
- **AND** Trace 树 SHALL 按新的 Task 范围刷新
- **AND** overlay SHALL 标记为未保存
...
### Requirement: 移动 Turn 到相邻 Task
Claude Log Viewer SHALL 允许用户将边界附近的 Turn 移动到上一个或下一个 Task，用于快速修正相邻 Task 边界。

#### Scenario: 移到上一个 Task
- **WHEN** 用户在编辑模式下选中某个 Task 的首部 Turn 并点击“移到上一个 Task”
- **THEN** 页面 SHALL 调整当前 Task 和上一个 Task 的边界
- **AND** 被选中 Turn SHALL 属于上一个 Task
- **AND** overlay SHALL 标记为未保存

#### Scenario: 移到下一个 Task
- **WHEN** 用户在编辑模式下选中某个 Task 的尾部 Turn 并点击“移到下一个 Task”
- **THEN** 页面 SHALL 调整当前 Task 和下一个 Task 的边界
- **AND** 被选中 Turn SHALL 属于下一个 Task
- **AND** overlay SHALL 标记为未保存

#### Scenario: 无相邻 Task
- **WHEN** 用户选中的 Turn 不存在可移动到的上一个或下一个 Task
- **THEN** 对应操作 SHALL disabled 或展示明确不可用提示

### Requirement: 拆分、合并和删除 Task
Claude Log Viewer SHALL 支持基础 Task 结构编辑，包括拆分、合并和删除。

#### Scenario: 从当前 Turn 拆分 Task
- **WHEN** 用户在编辑模式下选中某个 Task 内的 Turn 并点击“从这里拆分 Task”
- **THEN** 页面 SHALL 将原 Task 拆为两个连续 Task
- **AND** 新 Task SHALL 从选中 Turn 开始
...
- **AND** 被删除 Task 覆盖的 Turn SHALL 进入 unassigned 状态或被相邻 Task 接管
...
Claude Log Viewer SHALL 允许用户不依赖自动切分，直接从 Trace 树中选择 Turn 范围创建 Task。

#### Scenario: 进入手动创建模式
- **WHEN** 用户点击“手动创建 Task”
- **THEN** 页面 SHALL 进入手动创建模式
- **AND** 页面 SHALL 引导用户选择起始 Turn 和结束 Turn

#### Scenario: 创建手动 Task
- **WHEN** 用户选择有效起始 Turn、结束 Turn、标题和类型并点击“创建”
- **THEN** 页面 SHALL 创建或更新 active Task Trace Overlay
- **AND** 新 Task SHALL 覆盖选定的连续 Turn 范围
- **AND** overlay source SHALL 为 `manual` 或 `edited`
- **AND** Trace 树 SHALL 刷新为 Task-first 结构

#### Scenario: 手动范围无效
- **WHEN** 用户选择的 Turn 范围为空、顺序错误或跨越当前版本不支持的范围
...
- **THEN** 页面 SHALL 导出包含 sessionId、overlayId、source、revision、tasks、start/end Turn 和 start/end event anchor 的 JSON
