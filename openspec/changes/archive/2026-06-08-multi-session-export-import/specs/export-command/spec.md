...
### Requirement: 导出文件名包含 session 短 ID 或 session 数量
默认文件名格式 SHALL 根据导出 session 数量变化：单 session 时为 `export-YYYYMMDD-HHmmss-<session-id前8位>.tar.gz`，多 session 时为 `export-YYYYMMDD-HHmmss-<count>-sessions.tar.gz`。

#### Scenario: 单 session 导出时默认文件名包含时间戳和短 ID
- **WHEN** 导出 session `c821b9c9-48e7-4383-94e0-09a29fb75d38`
- **THEN** 默认文件名为类似 `export-20260526-120017-c821b9c9.tar.gz` 的格式

#### Scenario: 多 session 导出时默认文件名包含 session 数量
- **WHEN** 同时导出 2 个 session 且未指定输出文件名
- **THEN** 默认文件名为类似 `export-20260527-201530-2-sessions.tar.gz` 的格式
...
- **AND** 当导出包包含多个 session 时，导入命令仍指向同一个包文件
