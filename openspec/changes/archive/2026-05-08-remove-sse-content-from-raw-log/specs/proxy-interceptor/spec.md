...
### Requirement: sse_content field present in SSE record
**Reason**: `sse_content` 与 `response.body` 内容重复，增加日志冗余，从原始 JSONL 记录中移除
**Migration**: 消费方改用 `response.body` 字段获取 SSE 完整内容
