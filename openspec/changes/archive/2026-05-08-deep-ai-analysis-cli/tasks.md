...
- [x] 3.2 在 `deep_ai_analysis/config.py` 中定义 `RECORD_DOMAINS: list[str] = ["mcli.sankuai.com"]`，供 addon 导入使用
...
- [x] 6.2 启动 `deep-ai-analysis proxy`，配置 curl 使用代理，执行请求到 `mcli.sankuai.com`，验证 `logs/YYYY-MM-DD.jsonl` 生成且每条记录是合法的单行 JSON
