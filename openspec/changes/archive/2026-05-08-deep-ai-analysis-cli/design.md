...
- 域名过滤列表在代码中配置（默认 `["mcli.sankuai.com"]`，支持数组），不作为 CLI 参数
...
每行一条完整记录：
```jsonl
{"timestamp":"2026-05-08T10:30:00.123Z","domain":"mcli.sankuai.com","method":"POST","url":"https://mcli.sankuai.com/v1/chat","request":{"headers":{"content-type":"application/json"},"body":"{...}"},"response":{"status":200,"headers":{"content-type":"text/event-stream"},"body":"data: {...}\n\n"},"is_sse":true,"sse_events":["data: {\"type\":\"delta\",...}","data: [DONE]"]}
```
