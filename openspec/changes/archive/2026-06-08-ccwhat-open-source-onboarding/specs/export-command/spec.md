...
### Requirement: 默认导出路径为 ~/Downloads/ccwhat-exports/
CLI export 命令 SHALL 在未指定 `--output` 时，将文件保存到 `~/Downloads/ccwhat-exports/` 目录下，不再使用当前工作目录。

#### Scenario: 未指定 --output 时使用默认路径
- **WHEN** 用户执行 `ccwhat export <session-id>`（不带 -o）
- **THEN** 文件保存到 `~/Downloads/ccwhat-exports/export-YYYYMMDD-HHmmss-<短ID>.tar.gz`
- **AND** 若目录不存在则自动创建

#### Scenario: 指定 --output 时优先使用指定路径
- **WHEN** 用户执行 `ccwhat export <session-id> -o ./my-export.tar.gz`
...
- **WHEN** 导出成功完成
- **THEN** 终端输出文件路径
- **AND** 输出 `ccwhat import <文件路径> --open` 的完整命令供复制
