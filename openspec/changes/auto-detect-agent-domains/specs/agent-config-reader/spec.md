...
`agent_config` 模块 SHALL 提供 `detect_domains(agent_name: str) -> list[str]` 函数，根据 agent 名称读取其在 `Path.home()` 下固定路径的配置文件，提取所有 API provider 的 baseURL host，返回去重后的 domain 列表。当配置文件不存在或解析失败时，SHALL 静默返回该 agent 类型对应的默认 domain，不抛出异常、不打印错误。
...
- **THEN** `detect_domains("opencode")` 返回 `["api.anthropic.com"]`，不报错

#### Scenario: opencode 配置文件解析失败
- **WHEN** `~/.config/opencode/opencode.jsonc` 存在但内容无法解析
- **THEN** `detect_domains("opencode")` 返回 `["api.anthropic.com"]`，不抛出异常
