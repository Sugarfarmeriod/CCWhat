# port-bindability-diagnostics Specification

## Purpose
定义 ccwhat 在启动本机代理或 viewer 前对端口可绑定性的诊断行为，确保 Windows TCP excluded port range 等“没有 listener 但不可 `bind()`”的场景能给出可操作错误提示。
## Requirements
