# export-web-ui Specification

## Purpose
TBD - created by archiving change portable-diagnostic-export. Update Purpose after archive.
## Requirements
### Requirement: 导出弹窗支持 Session 选择
Web UI 导出弹窗 SHALL 提供 session 下拉选择器，打开时从 `/api/projects` 加载所有可用 session，默认选中当前正在查看的 session。

#### Scenario: 打开弹窗时预填当前 session
- **WHEN** 用户点击导出按钮
- **THEN** 弹窗内 session 选择器自动选中主页面当前查看的 session

#### Scenario: 可在弹窗内切换 session
- **WHEN** 用户在弹窗内选择不同 session
- **THEN** 文件名输入框自动更新为新 session 对应的默认文件名
...
- **THEN** 生成的压缩包不包含 req-resp/ 目录
- **AND** manifest.json 中 `included.reqResp` 为 `false`
